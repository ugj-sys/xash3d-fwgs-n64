...
// FIXME: use bounding-box-based frustum clipping info?
	...
	// outside the loop, and overflow checking must be done here
		if( !pbverts ) {
			pbverts = (mvertex_t *)Mem_Malloc( r_temppool, MAX_BMODEL_VERTS * sizeof(mvertex_t) );
			pbedges = (bedge_t *)Mem_Malloc( r_temppool, MAX_BMODEL_EDGES * sizeof(bedge_t) );
		}
		numbverts = numbedges = 0;
...
void R_RecursiveWorldNode (mnode_t *node, int clipflags)
{
	...
	/* QUARANTINED: FORCE RENDER ALL NODES
...
//  twice as fast in ASM
	/* NO-OP: Frustum culling bypassed to allow memory condensation on N64
	if (clipflags)
	{
		for (i=0 ; i<4 ; i++)
		{
	...
	// draw stuff
		...
		if (c)
		{
			...
			if (dot < -BACKFACE_EPSILON)
			{
				do
				{
					if (surf->flags & SURF_PLANEBACK) // FORCED: VISFRAME BYPASSED
					{
						R_RenderFace (surf, clipflags); // RESTORED FRUSTUM CLIPPING
					}
