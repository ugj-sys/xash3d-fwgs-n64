...
#define MAX_DECAL_SURFS	4096
	...
	#define MAX_TEXTURES 384	// HARVESTED: 384 concurrent textures releases 42KB static memory.
...
#if XASH_N64
	// On N64, we use pointers to avoid 2.6MB of static BSS.
	// These MUST be allocated dynamically before the renderer uses them.
	pixel_t                *colormap;               // 8192 * light levels (allocated at init)
	pixel_t				   *screen;                 // intermediate blit buffer
	unsigned int		   *screen32;               // intermediate blit buffer (32bpp)
	pixel_t                *alphamap;               // translucency map (allocated at init)
#else
...
#define NUMSTACKEDGES           4	// N64 SHRINK: Consumes almost 0 stack space
#define MINEDGES                        4000	// Preserves Full Capacity via HEAP!
#define NUMSTACKSURFACES        4	// N64 SHRINK: Consumes almost 0 stack space
#define MINSURFACES                     2000	// Preserves Full Capacity via HEAP!
...
#define CACHESPOT(surf) ((surfcache_t**)surf->info->surfcache)
...
#if XASH_N64
static inline pixel_t BLEND_ALPHA_N64(int alpha, pixel_t src, pixel_t dst)
{
	unsigned int r1 = (src >> 11) & 0x1F;
	unsigned int g1 = (src >> 5) & 0x3F;
	unsigned int b1 = src & 0x1F;

	unsigned int r2 = (dst >> 11) & 0x1F;
	unsigned int g2 = (dst >> 5) & 0x3F;
	unsigned int b2 = dst & 0x1F;

	unsigned int r = (r1 * alpha + r2 * (7 - alpha)) / 7;
	unsigned int g = (g1 * alpha + g2 * (7 - alpha)) / 7;
	unsigned int b = (b1 * alpha + b2 * (7 - alpha)) / 7;

	return (pixel_t)((r << 11) | (g << 5) | b);
}
#define BLEND_ALPHA(alpha, src, dst) BLEND_ALPHA_N64(alpha, src, dst)
#else
