...
#endif
	// Fill with 0x77 (Patterned calibration color) instead of 0
	memset( vid.buffer, 0x77, vid.width * vid.height * 2);
}
...
void R_MarkLeaves( void )
{
	...
	if( RI.viewleaf != NULL )
	{
		...
		if( leaf && ( leaf->contents != CONTENTS_SOLID ) && ( RI.viewleaf != leaf ))
			force = true;
	}
...
void GAME_EXPORT R_RenderScene( void )
{
	...
	// R_SetupGL( true );
	R_Clear( ~0 ); // DEFINITIVE VISUAL VALIDATION: RE-ENABLED.
	R_MarkLeaves();
	R_EdgeDrawing ();
...
#if XASH_N64
	r_fullbright = gEngfuncs.Cvar_Get( "r_fullbright", "1", FCVAR_CHEAT, "disable lightmaps, get fullbright for entities" );
#else
...
#if XASH_N64
	sw_drawflat = gEngfuncs.Cvar_Get ("sw_drawflat", "1", 0, "");
#else
