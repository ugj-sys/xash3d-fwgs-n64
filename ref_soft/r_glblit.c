...
#if XASH_N64
	// Halved to 16 brightness steps to recover 262KB heap.
	if( !vid.colormap ) vid.colormap = calloc( 16 * 8192, sizeof(pixel_t) );
	if( !vid.screen )   vid.screen = calloc( 256 * 256, sizeof(pixel_t) );
	// vid.screen32 disabled on N64 (Saves 262KB RAM)
	// if( !vid.screen32 ) vid.screen32 = calloc( 256 * 256, sizeof(unsigned int) );
#if 0
	if( !vid.alphamap ) vid.alphamap = calloc( 3 * 1024 * 256, sizeof(pixel_t) );
#endif
	if( !vid.addmap )   vid.addmap = calloc( 256 * 256, sizeof(byte) );
	if( !vid.modmap )   vid.modmap = calloc( 256 * 256, sizeof(byte) );
#endif
...
void R_AllocScreen( void )
{
	...
	vid.buffer = calloc( vid.width * vid.height, sizeof( pixel_t ) );
}
