...
void R_MarkLights( dlight_t *light, int bit, mnode_t *node )
{
	...
	{
		qboolean intersect = false;
		if (surf->numedges > 0 && RI.currentmodel && RI.currentmodel->surfedges && RI.currentmodel->vertexes && RI.currentmodel->edges)
		{
			vec3_t center = {0, 0, 0};
			float r_sq = 0.0f;
			int j;
			...
			{
				int lidx = RI.currentmodel->surfedges[surf->firstedge + j];
				medge_t *ed = &RI.currentmodel->edges[lidx < 0 ? -lidx : lidx];
				mvertex_t *v = &RI.currentmodel->vertexes[ed->v[0]];
				VectorAdd(center, v->position, center);
			}
			float inv_count = 1.0f / (float)surf->numedges;
			VectorScale(center, inv_count, center);
			...
			for (j = 0; j < surf->numedges; j++)
			{
				...
				mvertex_t *v = &RI.currentmodel->vertexes[ed->v[0]];
				vec3_t dir;
				VectorSubtract(v->position, center, dir);
				float d_sq = DotProduct(dir, dir);
				if (d_sq > r_sq) r_sq = d_sq;
			}
			
			vec3_t light_to_center;
			VectorSubtract(light->origin, center, light_to_center);
			float l_c_dist_sq = DotProduct(light_to_center, light_to_center);
			float combined_r = light->radius + (float)sqrt(r_sq);
			if (l_c_dist_sq <= combined_r * combined_r)
			{
				intersect = true;
			}
		}
...
static qboolean R_RecursiveLightPoint( model_t *model, mnode_t *node, float p1f, float p2f, colorVec *cv, const vec3_t start, const vec3_t end )
{
	...
	for( i = 0; i < node->numsurfaces; i++, surf++ )
	{
		...
		/* NO-OP: Deluxemap disabled to free 250KB space.
