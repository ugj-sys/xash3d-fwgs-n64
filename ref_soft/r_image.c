...
static qboolean GL_UploadTexture( image_t *tex, rgbdata_t *pic )
{
	...
	int x, y;
		...
		for(i = 0; i < height * width; i++ )
		{
				...
				unsigned char input_r, input_g, input_b, input_a = 255;
				qboolean isTransparent = false;

				if( ImageIND( pic->type ) )
				{
					int idx = data[i];
					int palStride = (pic->type == PF_INDEXED_32) ? 4 : 3;
					if( idx == 0xFF ) {
						isTransparent = true;
					}
					...
					if( pic->palette )
					{
						input_r = pic->palette[idx * palStride + 0];
						input_g = pic->palette[idx * palStride + 1];
						input_b = pic->palette[idx * palStride + 2];
						if( palStride == 4 ) input_a = pic->palette[idx * palStride + 3];
					}
					else
					{
						// Fallback 
						input_r = input_g = input_b = idx;
					}
				}
				else
				{
				...
				// FORCE TRANSPARENCY ON N64 HARDWARE CHANNEL
				if( isTransparent )
					tex->pixels[j][i] = TRANSPARENT_COLOR;

				if( j == 0 && tex->alpha_pixels )
				{
					unsigned int alpha = (input_a * 8 / 256) << (16 - 3);
					tex->alpha_pixels[i] = (tex->pixels[j][i] >> 3) | alpha;
					if( !sw_noalphabrushes->value && input_a < 128 && FBitSet( pic->flags, IMAGE_ONEBIT_ALPHA ) )
						tex->pixels[j][i] = TRANSPARENT_COLOR; 
				}
		}
...
static void GL_ProcessImage( image_t *tex, rgbdata_t *pic )
{
	...
	else
	{
		...
		// N64 OPTIMIZATION: Do not force expand to RGBA. Our native renderer handles indexed data directly.
