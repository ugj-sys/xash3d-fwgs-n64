...
void R_InsertNewEdges (edge_t *edgestoadd, edge_t *edgelist)
{
	...
	do
	{
		...
		int _limit1 = 0;
edgesearch:
		if (++_limit1 > 5000) { gEngfuncs.Con_Printf("InsertEdges HUNG!\n"); break; }
		if( !edgelist )
		{
...
void R_StepActiveU (edge_t *pedge)
{
	...
	while (1)
	{
		int _limit2 = 0;
nextedge:
		if (++_limit2 > 5000) { gEngfuncs.Con_Printf("StepActiveU HUNG!\n"); return; }
...
pushback:
	...
	//	if( !pwedge )
		...
		{
			pwedge = pwedge->prev;
			if (++_limit3 > 5000) { gEngfuncs.Con_Printf("Pwedge Walk HUNG!\n"); break; }
		}
...
continue_search:
		...
		{
			surf2 = surf2->next;
			if (++_limit4 > 5000) { gEngfuncs.Con_Printf("LeadingEdgeBack Walk HUNG!\n"); break; }
...
continue_search:
			...
			{
				surf2 = surf2->next;
				if (++_limit5 > 5000) { gEngfuncs.Con_Printf("LeadingEdge Walk HUNG!\n"); break; }
			} while (surf->key > surf2->key);
...
==============
*/
static byte *basespans_heap = NULL;
...
void R_ScanEdges (void)
{
	...
	if( !basespans_heap )
	{
		basespans_heap = (byte *)Mem_Malloc( r_temppool, MAXSPANS * sizeof(espan_t) + CACHE_SIZE );
	}

	basespan_p = (espan_t *)
			((long)(basespans_heap + CACHE_SIZE - 1) & ~(CACHE_SIZE - 1));
