...
void SV_RejectConnection( netadr_t from, char *fmt, ... )
{
	...
	Con_Printf( "^1REJECTED:^7 %s. Reason: %s\n", NET_AdrToString( from ), text );
...
{
	byte *msg_buf = (byte*)Z_Malloc( 0x20200 );	// MAX_INIT_MSG + some space
	edict_t		*ent = cl->edict;
	sizebuf_t		msg;

	MSG_Init( &msg, "Spawn", msg_buf, 0x20200 );
...
static qboolean SV_New_f( sv_client_t *cl )
{
	...
	msg_buf = (byte *)Z_Malloc( MAX_INIT_MSG );
	MSG_Init( &msg, "New", msg_buf, MAX_INIT_MSG );
...
static qboolean SV_SendRes_f( sv_client_t *cl )
{
	...
	MSG_Init( &msg, "SendResources", buffer, MAX_INIT_MSG );
