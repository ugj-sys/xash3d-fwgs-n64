...
static void CL_CopyEntityToPhysEnt( physent_t *pe, entity_state_t *state, qboolean visent )
{
	...
	else
	{
		...
		if( mod ) Q_strncpy( pe->name, mod->name, sizeof( pe->name ));
		else Q_strncpy( pe->name, "nullmodel", sizeof( pe->name ));
	}
	...
	{
		if( mod && FBitSet( mod->flags, STUDIO_TRACE_HITBOX ))
			pe->studiomodel = mod;
	}
	else
	{
	...
	if( visent && VectorIsNull( pe->mins ) && mod )
	{
