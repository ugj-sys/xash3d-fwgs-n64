...
static int CL_Stub_Func( void ) { return 1; }
...
void CL_InitEdicts( void )
{
...
#endif
#if XASH_N64 || XASH_LOW_MEMORY
	if( clgame.maxEntities > 256 ) clgame.maxEntities = 256; // ABSOLUTE N64 CAP: 256 to fit in 8MB RDRAM
#endif
	cls.num_client_entities = cl.maxclients * CL_UPDATE_BACKUP * clgame.maxEntities;
	...
	{
		clgame.entities[i].index = i;
		clgame.entities[i].trivial_accept = INVALID_HANDLE;
	}

	Mem_PrintStats(); // DIAGNOSTIC FOR N64 OOM
...
static qboolean CL_LoadHudSprite( const char *szSpriteName, model_t *m_pSprite, uint type, uint texFlags )
{
	...
	if( !FS_FileExists( szSpriteName, false ) )
	{
		...
		{
			Con_Reportf( S_WARN "Fabricating Dummy Sprite for missing: %s\n", szSpriteName );
			m_pSprite->type = mod_sprite;
			m_pSprite->mempool = Mem_AllocPool( va( "^2%s^7", m_pSprite->name ));
			msprite_t *psprite = (msprite_t *)Mem_Calloc( m_pSprite->mempool, sizeof( msprite_t ) );
			m_pSprite->cache.data = psprite;
			psprite->numframes = m_pSprite->numframes = 0; // Dedicated server style: 0 frames, no rendering!
			m_pSprite->needload = NL_PRESENT;
			return true; // DECEIVED! The engine now believes it's real!
		}
	}
...
qboolean CL_LoadProgs( const char *name )
{
	...
	if( !clgame.hInstance )
	{
		...
		for( func = cdll_exports; func && func->name; func++ )
			*func->func = (void*)CL_Stub_Func;
		for( func = cdll_new_exports; func && func->name; func++ )
			*func->func = (void*)CL_Stub_Func;
		return true; // N64 Force Continue
#else
		return false;
#endif
	}
