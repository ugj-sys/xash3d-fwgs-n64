...
qboolean CL_RequestMissingResources( void )
{
	...
	if( !cls.dl.doneregistering && ( cls.dl.custom || cls.state == ca_validate ))
	{
		...
		{
			if( !p ) {
				Con_Printf("^1NULL RESOURCE IN LIST DETECTED!^7\n");
				return false;
			}
			// Log a warning but only once per request to avoid spam
			// Con_Printf("[RES] Moving %s to on-hand list.\n", p->szFileName);
...
{
	byte		*data = (byte *)Z_Malloc( MAX_INIT_MSG );
	resource_t	*p, *n;
	sizebuf_t		msg;

	MSG_Init( &msg, "Resource Batch", data, MAX_INIT_MSG );
...
void CL_ParseResourceRequest( sizebuf_t *msg )
{
	...
	buffer = (byte *)Z_Malloc( MAX_INIT_MSG );
	MSG_Init( &sbuf, "ResourceBlock", buffer, MAX_INIT_MSG );
