...
typedef struct con_lineinfo_s
{
	...
	size_t		length;
#if XASH_N64
	float		addtime;	// double at offset 8 causes misaligned sdc1 on MIPS; float is safe
#else
...
static qboolean Con_LoadFixedWidthFont( const char *fontname, cl_font_t *font )
{
	...
	R_GetTextureParms( &fontWidth, &fontHeight, font->hFontTexture );
	if( fontWidth <= 0 ) fontWidth = 256; // N64 Guard
	if( fontHeight <= 0 ) fontHeight = 128; // N64 Guard

	if( font->hFontTexture && fontWidth != 0 )
	{
		font->charHeight = fontHeight / 16 * con_fontscale->value;
		...
		for( i = 0; i < 256; i++ )
		{
			...
			font->fontRc[i].top = (i / 16) * (fontHeight / 16);
			font->fontRc[i].bottom = font->fontRc[i].top + fontHeight / 16;
...
static qboolean Con_LoadVariableWidthFont( const char *fontname, cl_font_t *font )
{
	...
	if( font->hFontTexture && fontWidth != 0 )
	{
		...
		if( buffer && length >= sizeof( qfont_t ))
		{
			...
			for( i = 0; i < 256; i++ )
			{
				font->fontRc[i].left = (word)LittleShort(src->fontinfo[i].startoffset) % fontWidth;
				font->fontRc[i].right = font->fontRc[i].left + LittleShort(src->fontinfo[i].charwidth);
				font->fontRc[i].top = (word)LittleShort(src->fontinfo[i].startoffset) / fontWidth;
...
static void Con_LoadConsoleFont( int fontNumber, cl_font_t *font )
{
	...
	// N64 FORCE: We must load the variable width font data from the header table!
