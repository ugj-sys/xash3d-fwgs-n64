...
void CL_ReadNetMessage( void )
{
	...
	while( CL_GetMessage( net_message_buffer, &curSize ))
	{
		...
		// SAFE Check: Explicitly match both raw and bit-packed representations discovered by telemetry
		qboolean isOOB = (MSG_GetMaxBytes( &net_message ) >= 4 && 
						  ((net_message.pData[0] == 0xFF && net_message.pData[1] == 0xFF && net_message.pData[2] == 0xFF && net_message.pData[3] == 0xFF) ||
						   (net_message.pData[0] == 0x7F && net_message.pData[1] == 0xFF && net_message.pData[2] == 0xFF && net_message.pData[3] == 0xFF)));
...
void CL_ProcessFile( qboolean successfully_received, const char *filename )
{
	...
	if( cls.state != ca_disconnected )
	{
		...
		{
			byte	*msg_buf = (byte *)Z_Malloc( MAX_INIT_MSG );
			sizebuf_t msg;

			MSG_Init( &msg, "Resource Registration", msg_buf, MAX_INIT_MSG );
...
//	Voice_Idle( host.frametime );
	...
	// processing resources on handle
	int resource_loop_count = 0;
	while( CL_RequestMissingResources( ) )
	{
		resource_loop_count++;
		if( resource_loop_count > 1000 )
		{
			Con_Printf("^1FATAL LOOP DETECTED in CL_RequestMissingResources! BREAKING LOOP.^7\n");
			break;
		}
	}
...
#include <malloc.h>
void PrintProbe(const char *tag) {
	struct mallinfo mi = mallinfo();
	Con_Printf("^2[HEAP PROBE] ^7%s -> ^1USED: %d KB^7\n", tag, mi.uordblks / 1024);
}
...
void CL_Init( void )
{
	...
	Con_Printf( "--- FINAL PHYSICAL ANATOMY CHECK ---\n" );
	Con_Printf( "sizeof(client_t) = %d\n", (int)sizeof(client_t) );
	Con_Printf( "OFF frames = %d\n", (int)offsetof(client_t, frames) );
	Con_Printf( "OFF players = %d\n", (int)offsetof(client_t, players) );
	Con_Printf( "OFF local = %d\n", (int)offsetof(client_t, local) );
	Con_Printf( "OFF instanced = %d\n", (int)offsetof(client_t, instanced_baseline) );
	Con_Printf( "OFF sound_p = %d\n", (int)offsetof(client_t, sound_precache) );
	Con_Printf( "OFF model_p = %d\n", (int)offsetof(client_t, models) );
	Con_Printf( "OFF consistency = %d\n", (int)offsetof(client_t, consistency_list) );
	Con_Printf( "OFF resourcelist = %d\n", (int)offsetof(client_t, resourcelist) );
	Con_Printf( "OFF decal_idx = %d\n", (int)offsetof(client_t, decal_index) );
	Con_Printf( "sizeof(cl.resourcelist) = %d\n", (int)sizeof(cl.resourcelist) );
	Con_Printf( "--- END ANATOMY CHECK ---\n" );
	...
	extern void PrintProbe(const char *tag);
	
	PrintProbe("BEFORE VID_INIT");
	VID_Init();	// init video
	PrintProbe("AFTER VID_INIT");
	S_Init();	// init sound
	PrintProbe("AFTER S_INIT");
	...
	PrintProbe("BEFORE CL_LOADPROGS");
	if( !CL_LoadProgs( libpath ) )
		Host_Error( "can't initialize %s: %s\n", libpath, COM_GetLibraryError() );
	PrintProbe("AFTER CL_LOADPROGS");

#if XASH_N64
	if (g_sd_mounted)
		Con_Printf("^2[SD STORAGE] ^7SUMMERCART64 MOUNTED -> ^2SWAP READY^7\n");
	else
		Con_Printf("^1[SD STORAGE] ^7NO FLASHCART DETECTED -> ^1SWAP DISABLED^7\n");
#endif
