...
qboolean SCR_LoadFixedWidthFont( const char *fontname )
{
	...
	R_GetTextureParms( &fontWidth, &fontHeight, cls.creditsFont.hFontTexture );
	if( fontWidth <= 0 ) fontWidth = 256; // N64 Zero Guard
	if( fontHeight <= 0 ) fontHeight = 128; // N64 Zero Guard
	cls.creditsFont.charHeight = clgame.scrInfo.iCharHeight = fontHeight / 16;
	...
	// build fixed rectangles
	{
		...
		fileBuffer = FS_LoadFile( fontname, &length, false );
		if( fileBuffer && length >= sizeof( qfont_t ) )
		{
			src = (qfont_t *)fileBuffer;
			cls.creditsFont.charHeight = clgame.scrInfo.iCharHeight = LittleLong(src->rowheight);
			cls.creditsFont.type = FONT_VARIABLE;
			
			for( i = 0; i < 256; i++ )
			{
				cls.creditsFont.fontRc[i].left = (word)LittleShort(src->fontinfo[i].startoffset) % fontWidth;
				cls.creditsFont.fontRc[i].right = cls.creditsFont.fontRc[i].left + LittleShort(src->fontinfo[i].charwidth);
				cls.creditsFont.fontRc[i].top = (word)LittleShort(src->fontinfo[i].startoffset) / fontWidth;
				cls.creditsFont.fontRc[i].bottom = cls.creditsFont.fontRc[i].top + LittleLong(src->rowheight);
				cls.creditsFont.charWidths[i] = clgame.scrInfo.charWidths[i] = LittleShort(src->fontinfo[i].charwidth);
			}
			cls.creditsFont.valid = true;
		}
		else
		{
			// fallback to fixed if load failed
			cls.creditsFont.type = FONT_FIXED;
			for( i = 0; i < 256; i++ )
			{
				...
				cls.creditsFont.fontRc[i].top = (i / 16) * (fontHeight / 16);
				cls.creditsFont.fontRc[i].bottom = cls.creditsFont.fontRc[i].top + fontHeight / 16;
...
qboolean SCR_LoadVariableWidthFont( const char *fontname )
{
	...
	if( buffer && length >= sizeof( qfont_t ))
	{
		...
		// build rectangles using Endian-swapped coordinates
		for( i = 0; i < 256; i++ )
		{
			cls.creditsFont.fontRc[i].left = (word)LittleShort(src->fontinfo[i].startoffset) % fontWidth;
			cls.creditsFont.fontRc[i].right = cls.creditsFont.fontRc[i].left + LittleShort(src->fontinfo[i].charwidth);
