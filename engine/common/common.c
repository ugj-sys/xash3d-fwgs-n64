...
uint LZSS_Decompress( const byte *pInput, byte *pOutput )
{
	...
	while( 1 )
	{
		...
		// SAFETY FAILSAFE: Prevent infinite hang on MIPS if stream is truncated or corrupted!
		if( totalBytes > actualSize ) 
		{
			// Explicit fallback abort, don't stay in while(1) forever
			return 0; 
		}
	}
...
char *_copystring( byte *mempool, const char *s, const char *filename, int fileline )
{
	...
	len = Q_strlen( s );
	b = _Mem_Alloc( mempool, len + 1, false, filename, fileline );
	memcpy( b, s, len );   // copy exactly len bytes — never overwrites sentinel2
	b[len] = '\0';
