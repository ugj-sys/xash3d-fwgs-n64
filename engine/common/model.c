...
model_t *Mod_LoadModel( model_t *mod, qboolean crash )
{
	...
	{
		uint magic = 0;
		file_t *peek = FS_Open( tempname, "rb", false );
		if( peek ) {
			FS_Read( peek, &magic, sizeof( magic ));
			magic = LittleLong( magic ); // Endian conversion for N64 match
			FS_Close( peek );
			Con_Printf("DEBUG: MODEL %s PEEK MAGIC=%08X\n", tempname, magic);
		}

		if( magic == HLBSP_VERSION || magic == Q1BSP_VERSION || magic == QBSP2_VERSION )
		{
			Con_Printf("DEBUG: ACTIVATING PIECEWISE STREAM FOR %s\n", tempname);
			// DO NOT load full map! Reserve transient micro-header buffer only.
			// 16KB is more than enough for the header table structure.
			length = 16384;
			buf = (byte *)Mem_Malloc( host.mempool, length );
			peek = FS_Open( tempname, "rb", false );
			if( peek ) {
				FS_Read( peek, buf, length );
				FS_Close( peek );
			}
		}
		else
