...
{
	fragbuf_t	*prev, *curr;
	int		pbuf_id = FRAG_GETID( pbuf->bufferid );
	...
	// Case 1: List is empty OR needs to be inserted at the head (sorted order)
	if( !*pplist || FRAG_GETID( (*pplist)->bufferid ) > pbuf_id )
	{
		pbuf->next = *pplist;
		*pplist = pbuf;
		return;
	...
	// Case 2: Traverse to find appropriate insertion point between elements
	prev = *pplist;
	curr = prev->next;

	while( curr )
	{
		if( FRAG_GETID( curr->bufferid ) > pbuf_id )
		{
	...
	// Insert 'pbuf' between 'prev' and 'curr' (which might be NULL)
	pbuf->next = curr;
	prev->next = pbuf;
}
...
static void Netchan_CreateFragments_( netchan_t *chan, sizebuf_t *msg )
{
	...
	// N64 OPTIMIZATION: Hard disable packet compression on local loopback.
	// Direct memory transfers are much faster than running MIPS LZSS cycles which are currently causing infinite loops on corrupt streams.
#if 0
...
fragbuf_t *Netchan_FindBufferById( fragbuf_t **pplist, int id, qboolean allocate )
{
	...
	{
		if( ++safety > 2000 ) {
			Con_Printf("^1[FATAL] Recursion detected in Netchan_FindBufferById!^7\n");
			break;
		}
		if( list->bufferid == id )
			return list;
...
void Netchan_CheckForCompletion( netchan_t *chan, int stream, int intotalbuffers )
{
	...
	{
		if( ++safety > 2000 ) {
			Con_Printf("^1[FATAL] Recursion detected in Netchan_CheckForCompletion!^7\n");
			break;
		}
...
qboolean Netchan_CopyNormalFragments( netchan_t *chan, sizebuf_t *msg, size_t *length )
{
	...
	{
		if( ++safety > 2000 ) {
			Con_Printf("^1[FATAL] Recursion detected in Netchan_CopyNormalFragments!^7\n");
			break;
		}
		n = p->next;
