...
void MSG_WriteUBitLong( sizebuf_t *sb, uint curData, int numbits )
{
	...
	// SAFE PORTABLE BITPACKER: Iterate through bits and pack sequentially at the byte level
	for( i = 0; i < numbits; i++ )
	{
		int bit = (curData >> i) & 1;
		if( bit ) 
			sb->pData[sb->iCurBit >> 3] |= (1 << (sb->iCurBit & 7));
		else 
			sb->pData[sb->iCurBit >> 3] &= ~(1 << (sb->iCurBit & 7));
...
/* DISABLED UNSAFE OPTIMIZATION ON BIG ENDIAN
	// read dwords.
	while( nBitsLeft >= 32 )
	{
...
uint MSG_ReadUBitLong( sizebuf_t *sb, int numbits )
{
	...
	// SAFE PORTABLE BIT-UNPACKER: Pull bits sequentially from the byte buffer
	for( i = 0; i < numbits; i++ )
	{
		int bit = (sb->pData[sb->iCurBit >> 3] >> (sb->iCurBit & 7)) & 1;
		if( bit )
			ret |= (1 << i);
		sb->iCurBit++;
	}
