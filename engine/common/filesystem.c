...
struct dirent {
    char d_name[256];
    int  d_type;  // DT_REG=1, DT_DIR=2
};
typedef struct {
    char path[256];   // normalized DragonFS path (rom:/...)
    dir_t dir;
    struct dirent ent;
    int is_first;
    int is_valid;
...
// Translate Xash paths ("./" relative) to DragonFS "rom:/" paths.
static void n64_normalize_path(const char *in, char *out, size_t outsz)
{
    // Strip leading "./"
    while( in[0] == '.' && in[1] == '/' ) in += 2;
    // Empty string or lone "." → DFS root
    if( in[0] == '\0' || (in[0] == '.' && in[1] == '\0') )
    {
        strncpy(out, "rom:/", outsz);
        return;
    }
    // Already prefixed
    if( strncmp(in, "rom:/", 5) == 0 )
        strncpy(out, in, outsz);
    else
        snprintf(out, outsz, "rom:/%s", in);
    // Strip trailing slash (except bare "rom:/")
...
static DIR *opendir(const char *name) {
    ...
    if(!d) return NULL;
    n64_normalize_path(name, d->path, sizeof(d->path));
    d->is_first = 1;
    d->is_valid = 1;
    return d;
}
static struct dirent *readdir(DIR *d) {
    ...
    int res;
    if(d->is_first) {
        res = dir_findfirst(d->path, &d->dir);
        d->is_first = 0;
    } else {
        res = dir_findnext(d->path, &d->dir);
    }
    if(res < 0) {
        d->is_valid = 0;
        return NULL;
    }
    strncpy(d->ent.d_name, d->dir.d_name, sizeof(d->ent.d_name)-1);
    d->ent.d_name[sizeof(d->ent.d_name)-1] = '\0';
    d->ent.d_type = (d->dir.d_type == DT_DIR) ? DT_DIR : DT_REG;
    return &d->ent;
}
...
#if XASH_N64
__attribute__((aligned(8)))  // MIPS sd/ld require 8-byte alignment for int64_t fields
#endif
;
...
{
#if XASH_N64
	// DragonFS is read-only — gameinfo.txt is pre-built into the DFS image.
	// Attempting to open for write crashes in FS_SysOpen (misaligned int64_t).
	return;
#endif
...
static int FS_SysFileTime( const char *filename )
{
	...
	struct stat buf;
	char npath[256];
	n64_normalize_path(filename, npath, sizeof(npath));
	if( stat( npath, &buf ) == -1 )
		return -1;
	return buf.st_mtime;
#else
...
static file_t *FS_SysOpen( const char *filepath, const char *mode )
{
	...
	if( mod != O_RDONLY )
	{
		// ROM is read only. Immediately abort attempting to write console logs or config files.
		Mem_Free( file );
		return NULL;
	}
	char npath[256];
	n64_normalize_path(filepath, npath, sizeof(npath));
	file->handle = open( npath, mod|opt, 0666 );
#else
...
qboolean FS_SysFileExists( const char *path, qboolean caseinsensitive )
{
	...
	struct stat buf;
	char npath[256];
	n64_normalize_path(path, npath, sizeof(npath));
	ret = stat( npath, &buf );
