...
void Mod_LoadStudioModel( model_t *mod, const void *buffer, qboolean *loaded )
{
	...
	// NUCLEAR GHOST PROTOCOL: Instantly fabricate a perfect native native-endian dummy
	Con_Printf( "^3N64 ULTIMATE BYPASS: Ghosting Model %s^7\n", mod->name );
	phdr = (studiohdr_t *)Mem_Calloc( loadmodel->mempool, sizeof(studiohdr_t) );
	phdr->ident = IDSTUDIOHEADER;
	phdr->version = STUDIO_VERSION;
	phdr->length = sizeof(studiohdr_t);
	phdr->bbmin[0] = -16; phdr->bbmin[1] = -16; phdr->bbmin[2] = -16;
	phdr->bbmax[0] =  16; phdr->bbmax[1] =  16; phdr->bbmax[2] =  16;
	
	loadmodel->cache.data = phdr;
	loadmodel->numframes = 1;
	loadmodel->radius = 32.0f;
	loadmodel->flags = 0;
	...
	return; // BYPASS ENTIRE DANGEROUS FUNCTION
