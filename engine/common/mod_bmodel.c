...
extern void Mem_PrintList( size_t minallocationsize );
...
{
	return; // Bounds vectors eradicated to conserve 250KB heap space
	/* 
	int	i, e;
	mvertex_t	*v;
...
{
	return; // FRAGMENTATION BLOCK: Conserves ~300KB and prevents 4,385 allocations
	/*
...
=================
*/
static void Mod_SetParent_r( mnode_t *node, mnode_t *parent, int depth )
{
	if( !node ) return;
	if( depth > 64 ) return; // ULTIMATE N64 FAILSAFE

	node->parent = parent;

	if( node->contents < 0 ) return; // it's leaf
	Mod_SetParent_r( node->children[0], node, depth + 1 );
	Mod_SetParent_r( node->children[1], node, depth + 1 );
}

static void Mod_SetParent( mnode_t *node, mnode_t *parent )
{
	Mod_SetParent_r( node, parent, 0 );
}
...
==================
*/
static void CountClipNodes_r_internal( mnode_t *nodes, hull_t *hull, int nodenum, int depth, byte *visited )
{
	if( nodenum < 0 || nodenum >= loadmodel->numnodes ) return; // ABSOLUTE BOUNDS / LEAF DISCARD
	if( depth > 64 ) return; // ULTIMATE N64 FAILSAFE
	if( visited[nodenum] ) return; // CRITICAL CIRCULAR
	visited[nodenum] = 1;
	...
	mnode_t *node = &nodes[nodenum];
	if( node->contents < 0 ) return; // It is a leaf, do not recurse further.

	// Safely calculate child node offsets for recursion
	if( node->children[0] ) {
		int next0 = node->children[0] - nodes;
		CountClipNodes_r_internal( nodes, hull, next0, depth + 1, visited );
	}
	if( node->children[1] ) {
		int next1 = node->children[1] - nodes;
		CountClipNodes_r_internal( nodes, hull, next1, depth + 1, visited );
	}
}

static void CountClipNodes_r( mnode_t *src, hull_t *hull, int nodenum )
{
	// CountClipNodes_r is ONLY used for Hull 0 which is mirrored from loadmodel->nodes
	// USE GLOBAL HOST MEMPOOL FOR GUARANTEED SAFETY ON TEMPORARY BUFFER
	byte *visited = Mem_Calloc( host.mempool, loadmodel->numnodes + 1 );
	CountClipNodes_r_internal( src, hull, nodenum, 0, visited );
	Mem_Free( visited );
}

static void CountClipNodes32_r_internal( dclipnode32_t *src, hull_t *hull, int nodenum, int depth, byte *visited )
{
	extern dbspmodel_t srcmodel;
	if( nodenum < 0 || nodenum >= srcmodel.numclipnodes ) return; // ABSOLUTE BOUNDS
	if( depth > 64 ) return; // ULTIMATE N64 FAILSAFE
	if( visited[nodenum] ) return; // CRITICAL CIRCULAR
	visited[nodenum] = 1;
	...
	CountClipNodes32_r_internal( src, hull, src[nodenum].children[0], depth + 1, visited );
	CountClipNodes32_r_internal( src, hull, src[nodenum].children[1], depth + 1, visited );
}

static void CountClipNodes32_r( dclipnode32_t *src, hull_t *hull, int nodenum )
{
	extern dbspmodel_t srcmodel;
	// dynamically sized exactly to active node list for absolute safety using GLOBAL HOST POOL
	byte *visited = Mem_Calloc( host.mempool, srcmodel.numclipnodes + 1 );
	CountClipNodes32_r_internal( src, hull, nodenum, 0, visited );
	Mem_Free( visited );
}
...
==================
*/
static int RemapClipNodes_r( dclipnode32_t *srcnodes, hull_t *hull, int nodenum, int depth, int *remapped )
{
	dclipnode32_t	*src;
	mclipnode_t	*out;
	int		i, c;
	...
	// leaf?
	if( nodenum < 0 )
		return nodenum; // PRESERVE LEAF CONTENTS (FIX REGRESSION)
	if( nodenum >= srcmodel.numclipnodes )
		return -1; // Force invalid leaf if out of bounds safety
	if( depth > 64 ) return -1; // ULTIMATE N64 FAILSAFE - BREAK CHAIN WITH LEAF

	// CRITICAL DEDUPLICATION & CIRCULAR REFERENCE PROTECTION
	if( remapped[nodenum] == -2 ) return -1; // DETECTED CYCLE, KILL IT WITH LEAF
	if( remapped[nodenum] >= 0 ) return remapped[nodenum]; // DETECTED SHARED NODE, REUSE IT

	remapped[nodenum] = -2; // Mark as VISITING (Temp Sentinel)
	...
	for( i = 0; i < 2; i++ )
		out->children[i] = RemapClipNodes_r( srcnodes, hull, src->children[i], depth + 1, remapped );

	remapped[nodenum] = c; // Mark as FINISHED with final index for caching
	return c;
}
...
=================
*/
static void __attribute__((noinline)) Mod_MakeHull0( void )
{
	mnode_t		*in, *child;
	mclipnode_t	*out;
	hull_t		*hull;
...
=================
*/
static void __attribute__((noinline)) Mod_SetupHull( dbspmodel_t *bmod, model_t *mod, byte *mempool, int headnode, int hullnum )
{
	...
	// remap clipnodes to 16-bit indexes using global host pool for scratch memory
	{
		int *remapped = (int *)Mem_Malloc( host.mempool, (bmod->numclipnodes + 1) * sizeof(int) );
		int k;
		for( k = 0; k <= bmod->numclipnodes; k++ ) remapped[k] = -1; // Initialize to UNVISITED

		RemapClipNodes_r( bmod->clipnodes_out, hull, headnode, 0, remapped );
		Mem_Free( remapped );
	}
}
...
=================
*/
static void __attribute__((noinline)) Mod_SetupSubmodels( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < mod->numsubmodels; i++ )
	{
		...
		CountClipNodes_r( loadmodel->nodes, &mod->hulls[0], bm->headnode[0] );
		...
		for( j = 0; i != 0 && j < mod->nummodelsurfaces && (mod->firstmodelsurface + j) < mod->numsurfaces; j++ )
		{
...
static void Mod_LoadSubmodels( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < bmod->numsubmodels; i++, in++, out++ )
	{
		...
		for( j = 0; j < 3; j++ ) {
			...
			in->origin[j] = LittleFloat(in->origin[j]);
		}
		for( j = 0; j < MAX_MAP_HULLS; j++ ) in->headnode[j] = LittleLong(in->headnode[j]);
		in->visleafs = LittleLong(in->visleafs);
		in->firstface = LittleLong(in->firstface);
		in->numfaces = LittleLong(in->numfaces);
#endif
		for( j = 0; j < 3; j++ )
		{
...
static void Mod_LoadPlanes( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < bmod->numplanes; i++, in++, out++ )
	{
		...
		{
#ifdef XASH_BIG_ENDIAN
			in->normal[j] = LittleFloat(in->normal[j]);
#endif
...
#ifdef XASH_BIG_ENDIAN
		in->dist = LittleFloat(in->dist);
		in->type = LittleLong(in->type);
#endif
		out->dist = in->dist;
		out->type = in->type;
	}
...
static void Mod_LoadEdges( dbspmodel_t *bmod )
{
	...
	if( bmod->version == QBSP2_VERSION )
	{
		...
		{
#ifdef XASH_BIG_ENDIAN
			in->v[0] = LittleLong(in->v[0]);
			in->v[1] = LittleLong(in->v[1]);
#endif
			out->v[0] = in->v[0];
			out->v[1] = in->v[1];
		}
	...
	else
	{
		...
		{
#ifdef XASH_BIG_ENDIAN
			in->v[0] = LittleShort(in->v[0]);
			in->v[1] = LittleShort(in->v[1]);
#endif
			out->v[0] = (word)in->v[0];
			out->v[1] = (word)in->v[1];
		}
...
static void Mod_LoadSurfEdges( dbspmodel_t *bmod )
{
...
#ifdef XASH_BIG_ENDIAN
	{
		...
		for( i = 0; i < bmod->numsurfedges; i++ )
			loadmodel->surfedges[i] = LittleLong(bmod->surfedges[i]);
	}
#else
...
static void Mod_LoadTextures( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < loadmodel->numtextures; i++ )
	{
...
#ifdef XASH_BIG_ENDIAN
		if (in->dataofs[i] != -1) in->dataofs[i] = LittleLong(in->dataofs[i]);
#endif
		...
		for( j = 0; j < 4; j++ ) mt->offsets[j] = LittleLong(mt->offsets[j]);
#endif
		...
		if( !Host_IsDedicated() )
		{
			...
			if(( r_wadtextures->value && bmod->wadlist.count > 0 ) || ( mt->offsets[0] <= 0 ))
			{
				...
				for( j = bmod->wadlist.count - 1; j >= 0; j-- )
				{
					...
					if( FS_FileExists( texpath, false ))
					{
						...
						tx->gl_texturenum = R_GetBuiltinTexture( REF_DEFAULT_TEXTURE ); // TEMPORARY FOR HEAP TEST
			...
			if( FBitSet( REF_GET_PARM( PARM_TEX_FLAGS, tx->gl_texturenum ), TF_HAS_LUMA ))
			{
				...
				if( mt->offsets[0] > 0 )
				{
					...
					tx->fb_texturenum = R_GetBuiltinTexture( REF_DEFAULT_TEXTURE );
				}
				else
				{
...
static void Mod_LoadTexInfo( dbspmodel_t *bmod )
{
	...
	{
#ifdef XASH_BIG_ENDIAN
		fin->texture_step = LittleShort(fin->texture_step);
		fin->max_extent = LittleShort(fin->max_extent);
		fin->groupid = LittleShort(fin->groupid);
#endif
	...
	for( i = 0; i < bmod->numtexinfo; i++, in++, out++ )
	{
		...
		for( j = 0; j < 2; j++ )
			for( k = 0; k < 4; k++ )
				in->vecs[j][k] = LittleFloat(in->vecs[j][k]);
		in->miptex = LittleLong(in->miptex);
		in->flags = LittleShort(in->flags);
		in->faceinfo = LittleShort(in->faceinfo);
#endif
		for( j = 0; j < 2; j++ )
			for( k = 0; k < 4; k++ )
...
#ifdef XASH_BIG_ENDIAN
			in->firstedge = LittleLong(in->firstedge);
			in->numedges = LittleLong(in->numedges);
			in->side = LittleLong(in->side);
			in->planenum = LittleLong(in->planenum);
			in->texinfo = LittleLong(in->texinfo);
			in->lightofs = LittleLong(in->lightofs);
#endif
...
#ifdef XASH_BIG_ENDIAN
			in->firstedge = LittleLong(in->firstedge);
			in->numedges = LittleShort(in->numedges);
			in->side = LittleShort(in->side);
			in->planenum = LittleShort(in->planenum);
			in->texinfo = LittleShort(in->texinfo);
			in->lightofs = LittleLong(in->lightofs);
#endif
			if(( in->firstedge + in->numedges ) > loadmodel->numsurfedges )
			{
...
static void Mod_LoadNodes( dbspmodel_t *bmod )
{
	...
	Con_Printf("CRITICAL DIAGNOSTIC: ALLOCATING NODES: COUNT = %d, SIZEOF = %d\n", bmod->numnodes, sizeof(*out));
	SCR_UpdateScreen();
	...
	for( i = 0; i < loadmodel->numnodes; i++, out++ )
	{
		if( bmod->version == QBSP2_VERSION )
		{
			...
			in->firstface = LittleLong(in->firstface);
			in->numfaces = LittleLong(in->numfaces);
			for( j = 0; j < 2; j++ ) in->children[j] = LittleLong(in->children[j]);
#endif
			for( j = 0; j < 3; j++ )
			{
			...
			for( j = 0; j < 2; j++ )
			{
				p = in->children[j];
				if( p >= 0 ) {
					// CRITICAL N64 GUARD: Prevent memory jump
					if( p >= loadmodel->numnodes ) {
						Con_Printf("^1CRITICAL ERROR: BSP2 Node %d child %d references INVALID NODE %d! TERMINATING BRANCH.^7\n", i, j, p);
						// SET TO SOLID LEAF (-1 index becomes leaf 0)
						p = -1; 
					}
				}
				...
				if( p >= 0 ) {
					...
					int leaf_idx = -1 - p;
					if( leaf_idx < 0 || leaf_idx >= loadmodel->numleafs ) {
						leaf_idx = 0;
					}
					out->children[j] = (mnode_t *)(loadmodel->leafs + leaf_idx);
				}
			}
		}
		else
		{
...
#ifdef XASH_BIG_ENDIAN
			for( j = 0; j < 3; j++ ) {
				in->mins[j] = LittleShort(in->mins[j]);
				in->maxs[j] = LittleShort(in->maxs[j]);
			}
			in->planenum = LittleLong(in->planenum);
			in->firstface = LittleShort(in->firstface);
			in->numfaces = LittleShort(in->numfaces);
			for( j = 0; j < 2; j++ ) in->children[j] = LittleShort(in->children[j]);
#endif
			for( j = 0; j < 3; j++ )
			{
				// out->minmaxs[j+0] = in->mins[j];
			...
			for( j = 0; j < 2; j++ )
			{
				...
				if( p >= 0 ) {
					// CRITICAL N64 GUARD: Prevent memory jump
					if( p >= loadmodel->numnodes ) {
						Con_Printf("^1CRITICAL ERROR: Node %d child %d references INVALID NODE %d! TERMINATING BRANCH.^7\n", i, j, p);
						// SET TO SOLID LEAF (-1 index becomes leaf 0)
						p = -1; 
					}
				}
...
static void Mod_LoadLeafs( dbspmodel_t *bmod )
{
	...
	for( i = 0; i < bmod->numleafs; i++, out++ )
	{
		if( bmod->version == QBSP2_VERSION )
		{
			...
			in->contents = LittleLong(in->contents);
			in->visofs = LittleLong(in->visofs);
			in->firstmarksurface = LittleLong(in->firstmarksurface);
			in->nummarksurfaces = LittleLong(in->nummarksurfaces);
#endif
			for( j = 0; j < 3; j++ )
			{
				// out->minmaxs[j+0] = in->mins[j];
		...
		else
		{
			...
			in->contents = LittleLong(in->contents);
			in->visofs = LittleLong(in->visofs);
			in->firstmarksurface = LittleShort(in->firstmarksurface);
			in->nummarksurfaces = LittleShort(in->nummarksurfaces);
#endif
			for( j = 0; j < 3; j++ )
			{
				// out->minmaxs[j+0] = in->mins[j];
...
static void Mod_LoadClipnodes( dbspmodel_t *bmod )
{
...
#ifdef XASH_BIG_ENDIAN
			in->planenum = LittleLong(in->planenum);
			in->children[0] = LittleLong(in->children[0]);
			in->children[1] = LittleLong(in->children[1]);
#endif
	...
	{
		byte	*in_ptr = (byte *)bmod->clipnodes;

		for( i = 0; i < bmod->numclipnodes; i++, out++ )
		{
			dclipnode_t	*in = (dclipnode_t *)in_ptr;
#ifdef XASH_BIG_ENDIAN
			in->planenum = LittleLong(in->planenum);
			in->children[0] = LittleShort(in->children[0]);
			in->children[1] = LittleShort(in->children[1]);
#endif
			out->planenum = in->planenum;
			...
			in_ptr += 8; // FORCE ABSOLUTE 8-BYTE BINARY ADVANCE
		}
	}
...
static void Mod_LoadLighting( dbspmodel_t *bmod )
{
	...
	case 1:
		...
		// {
			// DISABLING MONO LIGHTMAP INGEST:
	...
	case 3:	// load colored lighting
		// DISABLING LIGHTMAP INGEST TO FREE MEGA-BYTES: FAKING EMPTY
		loadmodel->lightdata = NULL; 
	...
	/* DISABLE THE LOOP ENTIRELY: loadmodel->lightdata is NULL, making this loop a guaranteed no-op that only crashes on bmod->surfaces dereference 
...
qboolean Mod_LoadBmodelLumps( const byte *mod_base, qboolean isworld )
{
...
#ifdef XASH_BIG_ENDIAN
	// Master Header Swapping Protocol for N64 Integration
	...
	if( extrahdr->id == IDEXTRAHEADER || extrahdr->id == LittleLong(IDEXTRAHEADER) )
	{
		LittleLongSW( extrahdr->id );
		LittleLongSW( extrahdr->version );
		for( i = 0; i < EXTRA_LUMPS; i++ )
		{
	...
	// STREAM LOADING PROTOCOL FOR N64
	{
		file_t *lumpfile = FS_Open( loadmodel->name, "rb", false );
		dlump_t *l;
		extern void Mem_PrintList( size_t minallocationsize );
		extern int get_memory_size(void);
		
		typedef struct { int total; int used; } local_heap_stats_t;
		extern void sys_get_heap_stats(local_heap_stats_t *stats);
		local_heap_stats_t stats;
		sys_get_heap_stats(&stats);
		
		Con_Printf("CRITICAL HW: PHYSICAL N64 RAM DETECTED = %d KB\n", get_memory_size() / 1024);
		Con_Printf("CRITICAL OS: NATIVE HEAP USED = %d KB / TOTAL = %d KB\n", stats.used / 1024, stats.total / 1024);
		Con_Printf("--- BEGIN ABSOLUTE HEAP INVENTORY ---\n");
		Mem_PrintList( 1024 ); // Print all allocations >= 1KB
		Con_Printf("--- END HEAP INVENTORY ---\n");

		if( !lumpfile ) return false;

		int max_lump_len = 0;
		int li;
		for( li = 0; li < HEADER_LUMPS; li++ ) {
			if( li == LUMP_LIGHTING ) continue; // SUPPRESSED: NEVER LOADED, EXCLUDE FROM SIZE
			if( header->lumps[li].filelen > max_lump_len )
				max_lump_len = header->lumps[li].filelen;
		}
		void *scratch_buffer = Mem_Malloc( host.mempool, max_lump_len + 16 );

		// Entities
		l = &header->lumps[LUMP_ENTITIES];
		if( l->filelen > 0 ) {
			bmod->entdata = (byte *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->entdata, l->filelen );
			bmod->entdatasize = l->filelen;
		...
		if( l->filelen > 0 ) {
			bmod->planes = (dplane_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->planes, l->filelen );
			bmod->numplanes = l->filelen / sizeof(dplane_t);
			Mod_LoadPlanes( bmod );
			bmod->planes = NULL;
		}
		...
		if( l->filelen > 0 ) {
			bmod->submodels = (dmodel_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->submodels, l->filelen );
			bmod->numsubmodels = l->filelen / sizeof(dmodel_t);
			Mod_LoadSubmodels( bmod );
			bmod->submodels = NULL;
		}
		Con_Printf("DEBUG: Submodels Loaded.\n");

		// Vertexes
		l = &header->lumps[LUMP_VERTEXES];
		if( l->filelen > 0 ) {
			bmod->vertexes = (dvertex_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->vertexes, l->filelen );
			bmod->numvertexes = l->filelen / sizeof(dvertex_t);
			Mod_LoadVertexes( bmod );
			bmod->vertexes = NULL;
		}
		Con_Printf("DEBUG: Vertexes Loaded.\n");
		SCR_UpdateScreen();

		// Edges
		l = &header->lumps[LUMP_EDGES];
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dedge32_t) : sizeof(dedge_t);
			bmod->numedges = l->filelen / sz;
			bmod->edges = (dedge_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->edges, l->filelen );
			Mod_LoadEdges( bmod );
			bmod->edges = NULL;
		}
		...
		if( l->filelen > 0 ) {
			bmod->surfedges = (dsurfedge_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->surfedges, l->filelen );
			bmod->numsurfedges = l->filelen / sizeof(dsurfedge_t);
		...
		if( l->filelen > 0 ) {
			bmod->textures = (dmiptexlump_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->textures, l->filelen );
			bmod->texdatasize = l->filelen;
		...
		if( l->filelen > 0 ) {
			bmod->visdata = (byte *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->visdata, l->filelen );
			bmod->visdatasize = l->filelen;
		...
		if( l->filelen > 0 ) {
			bmod->texinfo = (dtexinfo_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->texinfo, l->filelen );
			bmod->numtexinfo = l->filelen / sizeof(dtexinfo_t);
		...
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dface32_t) : sizeof(dface_t);
			bmod->surfaces = (dface_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->surfaces, l->filelen );
			bmod->numsurfaces = l->filelen / sz;
			Mod_LoadSurfaces( bmod );
			bmod->surfaces = NULL;
		}
		Con_Printf("DEBUG: Surfaces Loaded.\n");
		SCR_UpdateScreen();

		bmod->lightdata = NULL;
		bmod->deluxdata = NULL;
		bmod->shadowdata = NULL;
		Con_Printf("DEBUG: Lighting Loaded (SUPPRESSED).\n");

		// Mark Surfaces
		l = &header->lumps[LUMP_MARKSURFACES];
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dmarkface32_t) : sizeof(dmarkface_t);
			bmod->markfaces = (dmarkface_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->markfaces, l->filelen );
			bmod->nummarkfaces = l->filelen / sz;
		...
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dleaf32_t) : sizeof(dleaf_t);
			bmod->leafs = (dleaf_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->leafs, l->filelen );
			bmod->numleafs = l->filelen / sz;
			Mod_LoadLeafs( bmod );
			bmod->leafs = NULL;
		}
		Con_Printf("DEBUG: Leafs Loaded.\n");
		SCR_UpdateScreen();

		// Nodes
		{
			sys_get_heap_stats(&stats);
			Con_Printf("OS HEAP CHECK: USED=%d KB, TOTAL=%d KB\n", stats.used / 1024, stats.total / 1024);
			SCR_UpdateScreen();
		}

		l = &header->lumps[LUMP_NODES];
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dnode32_t) : sizeof(dnode_t);
			bmod->nodes = (dnode_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->nodes, l->filelen );
			bmod->numnodes = l->filelen / sz;
			Mod_LoadNodes( bmod );
			bmod->nodes = NULL;
		}
		...
		// Clipnodes
		l = &header->lumps[LUMP_CLIPNODES];
		if( l->filelen > 0 ) {
			size_t sz = (bmod->version == QBSP2_VERSION) ? sizeof(dclipnode32_t) : sizeof(dclipnode_t);
			bmod->clipnodes = (dclipnode_t *)scratch_buffer;
			FS_Seek( lumpfile, l->fileofs, SEEK_SET );
			FS_Read( lumpfile, bmod->clipnodes, l->filelen );
			bmod->numclipnodes = l->filelen / sz;
			Mod_LoadClipnodes( bmod );
			bmod->clipnodes = NULL;
		}
		Con_Printf("DEBUG: Clipnodes Loaded.\n");
		SCR_UpdateScreen();

		FS_Close( lumpfile );
		Mem_Free( scratch_buffer );
	}

	// preform some post-initalization
	Mod_MakeHull0 ();
	Con_Printf("DEBUG: Hull0 Completed.\n");

	Con_Printf("DEBUG: ALL BSP LOADERS FINISHED 100%% SUCCESSFULLY!\n");
	SCR_UpdateScreen();

	if( isworld )
		loadmodel = mod;		// RESTORE POINTER EARLY BEFORE CLEANUP TO AVOID POOL==NULL CRASH

	Mod_SetupSubmodels( bmod );
	Con_Printf("DEBUG: Mod_SetupSubmodels Done.\n");
	SCR_UpdateScreen();

	if( isworld )
	{
#if !XASH_DEDICATED
		// DISABLED ON N64: Debug Hulls are for PC-Dev mode and consume fragile heap
		// Mod_InitDebugHulls();	// FIXME: build hulls for separate bmodels (shells, medkits etc)
		Con_Printf("DEBUG: Mod_InitDebugHulls Bypassed Safely.\n");
		SCR_UpdateScreen();
